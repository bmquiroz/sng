...
def insert_hostname(data):
    service_owner, region, location, os_name, zone, lifecycle, role, \
        counter = get_values_for_hostname(data)
    hostname_string = region + location + os_name + zone + lifecycle + \
                      role + str(counter)
    hostname_data = HostName(None, service_owner, region, location, os_name, zone, lifecycle, role, counter)
